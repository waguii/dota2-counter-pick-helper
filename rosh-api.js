...
class RoshApi {
    ...
    async reset() {
        await this.page.evaluate(() => {
            ...
            let resetButton = buttons.find(button => button.textContent.includes('Reset'));
            if (resetButton) {
                resetButton.click();
            }
        });
    ...
    async clearHero(side, position) {
        ...
        await this.page.evaluate((sideNumber, position) => {
            const main = document.querySelector('main');
            if (main) {
    ...
    async typeHeroName(heroName, side, position) {
        const sideNumber = side === 'Radiant' ? 0 : 1;
